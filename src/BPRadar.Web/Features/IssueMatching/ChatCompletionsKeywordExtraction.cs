...
internal static class ChatCompletionsKeywordExtraction
{
    ...
    public static async Task<IReadOnlyList<string>> ReadKeywordsAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!response.IsSuccessStatusCode)
        {
            throw new HttpRequestException(
                $"{providerName} keyword extraction failed with HTTP {(int)response.StatusCode}.",
                null,
                response.StatusCode);
        }
        ...
        catch (JsonException exception)
        {
            throw new InvalidDataException(
                $"{providerName} returned an invalid completion response.",
                exception);
        }
        ...
        {
            throw new InvalidDataException(
                $"{providerName} returned no keyword extraction content.");
        }
        ...
        {
            throw new InvalidDataException(
                $"{providerName} returned invalid keyword extraction content.",
                exception);
        }
        ...
        {
            throw new InvalidDataException(
                $"{providerName} returned an invalid keyword extraction response.");
        }
